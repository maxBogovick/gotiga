<script lang="ts">
    ...
    async function exportData() {
        if (!confirm('Внимание! Это полностью перезапишет данные на сервере данными из локальной базы. Продолжить?')) return;
        
        try {
            message = { text: 'Экспорт релиза... Это может занять время.', type: 'info' };
            const res = await api.exportRelease();
            showMessage('Экспорт завершен: ' + res, 'success');
        } catch (e) {
    ...
    async function sync() {
        try {
            ...
            showMessage('Синхронизация успешна: ' + res, 'success');
            await loadFigurines();
        } catch (e) {
...
<div class="min-h-screen bg-[#0a0806] text-[#d4c5b0] font-cinzel p-8">
    
    <header class="flex justify-between items-center mb-12 border-b border-[#d4c5b0]/20 pb-6">
        <div>
            <h1 class="text-4xl font-gothic mb-2">Реестр Смотрителя</h1>
            <p class="text-xs tracking-[0.3em] text-[#8a7f70] uppercase">Панель управления коллекцией</p>
        </div>
        <div class="flex gap-4">
            ...
            <button onclick={sync} class="btn-gothic border-blue-900/40 text-blue-400">
                🔄 Sync
            </button>
            <button onclick={exportData} class="btn-gothic border-amber-900/40 text-amber-600">
                📦 Экспорт релиза
            </button>
            <button onclick={createNew} class="btn-gothic">
                ✚ Новый экспонат
            </button>
    ...
    <div class="grid grid-cols-12 gap-8">
        ...
        <aside class="col-span-3 space-y-2">
            <h2 class="text-xs uppercase tracking-widest text-[#8a7f70] mb-4">Архив записей</h2>
        ...
        <!-- Main: Editor -->
        <main class="col-span-9 bg-[#141210]/50 p-8 border border-[#d4c5b0]/10 relative">
            {#if selectedFigurine}
                <div in:fade>
                        ...
                        <div class="space-y-4">
                            ...
                            <div class="flex gap-4">
                                ...
                                <label class="block flex-1 pt-6">
                                    ...
                                    <span class="ml-2 text-sm text-[#d4c5b0]">Показывать в музее</span>
                                </label>
                            </div>
                        </div>
                    </div>
    ...
    {#if message.text}
        <div 
            fixed bottom-8 right-8 p-4 border transition-all
            class="{message.type === 'error' ? 'bg-red-950 border-red-800 text-red-200' : 'bg-green-950 border-green-800 text-green-200'}"
            in:slide={{ axis: 'x' }}
        >
            {message.text}
        </div>
