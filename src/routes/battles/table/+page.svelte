...
<style>
  ...
  /* Карта в клетке обязана назвать ширину САМА. `.slot` внутри `BattleCard`
     несёт `container-type: inline-size`, то есть сжатие по своей же полосе: её
     содержимое собственную ширину не считает, и флексовым элементом она
     схлопывается в ноль — карта стоит на месте и не видна вовсе. Ровно то же
     правило записано в сцене (`.figure-body > .slot`), и по той же причине:
     из клетки ширину берёт клетка, не карта. Ребёнок, а не потомок: у призрака
     (`.ghost`) ширина уже есть от `inset`. */
  .place > :global(.slot) {
    width: 100%;
  }
