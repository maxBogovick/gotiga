<script lang="ts">
    ...
    type Section = { title: string; paragraphs: string[]; id?: string; acquireLink?: boolean };

    let sections = $derived<Section[]>([
        { title: $t('rightsSectionWhatTitle'), paragraphs: [$t('rightsWhatBody')] },
        { title: $t('rightsSectionShareTitle'), paragraphs: [$t('rightsShareBody')] },
        { title: $t('rightsSectionNotTitle'), paragraphs: [$t('rightsNotBody')] },
        { title: $t('rightsSectionFindTitle'), paragraphs: [$t('rightsFindBody')] },
        { id: 'request', title: $t('rightsSectionRequestTitle'), paragraphs: [$t('rightsRequestBody')] },
        { title: $t('rightsSectionWorksTitle'), paragraphs: [$t('rightsWorksBody')], acquireLink: true },
    ]);
</script>

<svelte:head>
    <title>{$t('rightsTitle')} — {$brandName}</title>
    <meta name="description" content={$t('rightsMetaDescription')} />
    <meta property="og:site_name" content={$brandName} />
    <meta property="og:type" content="website" />
    <meta property="og:title" content="{$t('rightsTitle')} — {$brandName}" />
    <meta property="og:description" content={$t('rightsMetaDescription')} />
    <meta property="og:url" content="{SITE_URL}/rights" />
...
<div class="root">
    ...
    <div class="page">
        ...
        <header class="page-header" in:fly={{ x: -20, duration: 900, delay: 100, easing: cubicOut }}>
            ...
            <h1 class="page-title">{$t('rightsTitle')}</h1>
            <p class="page-subtitle">{$t('rightsUpdated')}</p>
        </header>

        <div class="body" in:fade={{ duration: 600, delay: 150 }}>
            <p class="intro">{$t('rightsIntro')}</p>
            ...
            {#each sections as section (section.title)}
                <section class="policy-section" id={section.id}>
                    ...
                    {/each}
                    {#if section.acquireLink}
                        <a class="inline-link" href="/acquire">{$t('navAcquire')} →</a>
                    {/if}
                </section>
            {/each}
            ...
            <section class="policy-section">
                ...
                <p class="section-text">{$t('rightsContactBody')}</p>
