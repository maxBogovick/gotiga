    ...
    <div class="w-1/3 border-r border-[#d4c5b0]/10 pr-6 flex flex-col">
        <div class="flex justify-between items-center mb-4">
            <h2 class="text-xl font-gothic text-[#d4c5b0]">
                {category === 'author' ? 'Мысли (Author)' : 'Верстак (Workshop)'}
            </h2>
            <button onclick={createNew} class="btn-gothic text-xs">✚ Добавить</button>
        </div>
    ...
    <div class="flex-1">
        {#if selectedItem}
            <div class="bg-[#141210]/50 p-6 border border-[#d4c5b0]/10 h-full flex flex-col" in:fade>
                <h3 class="font-bold text-[#d4c5b0] border-b border-[#d4c5b0]/20 pb-4 mb-6">Редактирование</h3>

                <div class="space-y-6 flex-1">
                    <label class="block">
                        <span class="label">Текст / Содержание</span>
                        <textarea bind:value={selectedItem.content} class="input-gothic h-32 italic"></textarea>
                    </label>

                    {#if category === 'workshop'}
                        <label class="block">
                            <span class="label">Подпись (Caption)</span>
                            <input bind:value={(selectedItem as WorkshopItem).caption} class="input-gothic" />
                        </label>

                        <div>
                            <span class="label block mb-2">Изображение</span>
                            <div class="flex gap-4 items-start">
                                <div class="w-32 h-32 bg-black border border-[#d4c5b0]/20 flex items-center justify-center overflow-hidden">
                                    ...
                                    {:else}
                                        <span class="text-[#8a7f70] text-xs">Нет фото</span>
                                    {/if}
                                </div>
                                <div class="space-y-2">
                                    <button onclick={handlePickImage} class="btn-gothic w-full">Загрузить фото</button>
                                    {#if (selectedItem as WorkshopItem).imageUrl}
                                        <button 
                ...
                <div class="pt-6 flex justify-end gap-4">
                    ...
                    <button onclick={save} class="btn-gothic bg-[#d4c5b0]/10 text-[#d4c5b0] min-w-[120px]">Сохранить</button>
                </div>
            </div>
        {:else}
