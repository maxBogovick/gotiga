...
<div class="sc-root">
  ...
  >
    ...
    <div class="sc-overlay" aria-hidden="true"></div>

    <!-- Editorial identity bar: кикер + заголовок -->
    <div class="sc-editorial">
      <p class="sc-ed-kicker">
        <span class="sc-ed-kind">{$t('detailKind')}</span>
        {#if ctx.figurine.year}
          <span class="sc-ed-sep" aria-hidden="true">·</span>
          <span class="sc-ed-year">{ctx.toRoman(ctx.figurine.year)}</span>
        {/if}
      </p>
      <h1 class="sc-ed-title" style={computeElementStyle(ctx.displayConfig, 'name')}>{ctx.figurine.name}</h1>
    </div>

    <!-- Dot nav (если несколько снимков) -->
    {#if ctx.sortedImages.length > 1}
      <nav class="sc-dots" aria-label={$t('figurineShowView')}>
        {#each ctx.sortedImages as _, i}
          <button
            type="button"
            class="sc-dot {i === ctx.activeImageIndex ? 'sc-dot--on' : ''}"
    ...
    <!-- Gallery buttons — direct children (требование shared CSS) -->
    <button
    ...
    <button
      type="button" class="gallery-mark" class:gallery-mark--marked={ctx.markTone}
      onclick={ctx.toggleMarkPicker}
      aria-label={ctx.markLabel} aria-expanded={ctx.markPickerOpen} title={ctx.markLabel}
    >
    ...
    {#if ctx.markPickerOpen}
      {#each ctx.markToneOptions as opt, i (opt.tone)}
        <button
          type="button" class="gallery-mark-option" class:gallery-mark-option--active={ctx.markTone === opt.tone}
  ...
  <div class="sc-body">
    ...
    <!-- Компактная горизонтальная полоса атрибутов под hero -->
    {#if ctx.hasAttributesSection}
      <div class="sc-specs-bar" aria-label={$t('figurineAttributes')}>
        {#each ctx.attributes as attr (attr.kind)}
          <div class="sc-spec">
            <span class="sc-spec-label">{attr.label}</span>
            <span class="sc-spec-value">{attr.value}</span>
          </div>
        {/each}
      </div>
    {/if}

    <!-- Lead: только вступительный текст, без сайдбара -->
    {#if ctx.hasText(ctx.figurine.shortText) || (ctx.hasText(ctx.figurine.secretText) && ctx.isCandleLit)}
      <div class="sc-lead">
        {#if ctx.hasText(ctx.figurine.shortText)}
          <p class="sc-deck" style={computeElementStyle(ctx.displayConfig, 'shortText')}>{ctx.figurine.shortText}</p>
        {/if}
    ...
    <!-- Acquire: editorial CTA-полоса со статусом и записью -->
    <div class="sc-acquire">
      <div class="sc-acquire-panel">
        <FigurineStatusPanel
    ...
    {#if ctx.hasMakingSection}
      <section class="sc-section {ctx.figurine.status === 'in_progress' ? 'sc-section--live' : ''}">
        <header class="section-row">
          <span class="sec-label">
            {#if ctx.figurine.status === 'in_progress'}
              <span class="making-live" aria-hidden="true"></span>
            {/if}
            {ctx.figurine.status === 'in_progress' ? $t('detailMakingProgressKicker') : $t('detailMakingRecordKicker')}
          </span>
          <div class="sec-rule" aria-hidden="true"></div>
        </header>
        ...
        {#if ctx.hasBecoming}
          ...
          />
        {:else}
          <div class="sc-film-strip" aria-label={$t('detailMakingRecordTitle')}>
            {#each ctx.visibleProcessSteps.slice(0, 6) as step, i (step.id)}
              <article class="sc-frame">
                <div class="sc-frame-img">
                  {#if ctx.hasText(step.imageUrl)}
                    <img src={ctx.resolveUrl(step.imageUrl)} alt="" class="sc-frame-photo" loading="lazy" />
                  {:else}
                    <div class="sc-frame-blank" aria-hidden="true"></div>
                  {/if}
                  <span class="sc-frame-num">{String(i + 1).padStart(2, '0')}</span>
                </div>
                {#if step.description}
                  <p class="sc-frame-caption">{step.description}</p>
                {/if}
              </article>
            {/each}
          </div>
    ...
    {#if ctx.hasFactsSection && ctx.hasScheduleSection}
      <section id="presence" class="sc-section">
        <header class="section-row">
    ...
    {#if ctx.visibleRelatedItems.length > 0}
      <section id="related-works" class="sc-section">
        <header class="section-row">
        ...
        <div class="sc-related-grid">
          {#each ctx.visibleRelatedItems as item}
            ...
            <a href="/figurines/{item.id}"
              onclick={() => ctx.analyticsClient?.cta('related_figurine')}
              class="sc-related-card" data-sveltekit-preload-data="hover">
              <div class="sc-related-img">
                {#if relatedImageUrl}
                  <img src={relatedImageUrl} alt={item.name} class="sc-related-photo" loading="lazy" />
                {:else}
                  <div class="sc-related-blank" aria-hidden="true"><span>{item.name.slice(0,1)}</span></div>
                {/if}
                <span class="sc-related-badge sc-related-badge--{item.status}">{ctx.statusLabel(item.status)}</span>
              </div>
              <p class="sc-related-name">{item.name}</p>
              {#if item.year}<span class="sc-related-year">Anno {item.year}</span>{/if}
            </a>
          {/each}
        </div>
      </section>
