 ...
 * на карте — то же правило, по которому список деталей рамки задаёт слои: две
 * строки с одинаковым номером упали бы в порядок разметки, которого никто не
 * выбирал и никто не видит.
 *
 * Здесь нет ни координат, ни ширины. У текста длина меняется от языка и от
 * карты, и свободно поставленная плашка имени столкнётся с соседкой на первом
 * же длинном названии — поэтому текст размещается ПОЛОСОЙ и ПОРЯДКОМ, а
 * свободные координаты остаются у значков стоимости и силы, где стоит одна
 * цифра.
 */
export type SheetSlot =
    ...
    | 'effect'
    | 'lore'
    // Паспорт тела — семь строк, а не одна. «Здоровье в клетке боя, остальное
    // на листе взятия» невыразимо, пока паспорт неделим, а в клетке шириной
    // 140 px семь чисел не помещаются ни при каком кегле.
    | 'health'
    | 'mana'
    | 'armor'
    | 'ward'
    ...
    | 'mend'
    | 'stats'
    // Сами кружки. Они тоже печатаются на карте, и решать, видны ли они в
    // клетке боя, должна опись, а не медиазапрос. `healthMark` — кружок, а не
    // число паспорта: паспорт стоит строкой в полосе свойств, кружок висит
    // где поставили. Своего вида у него нет вовсе — он и ЕСТЬ кружок
    // стоимости, той же формы, заливки, величины и на том же месте, только с
    // другим числом: в клетке боя цена не значит ничего, а здоровье — всё.
    | 'cost'
    | 'power'
    | 'healthMark'
    | 'new'
...
/**
 * С какой ВЕЛИЧИНЫ строка печатается. Ступени названы по самой малой карте, на
 * которой строку ещё видно:
 *
 *   never    — не печатается вовсе
 *   large    — только лист взятия (от 281 px)
 *   always   — полка и крупнее (от 161 px)
 *   cell     — везде, включая клетку боя
 *   cellOnly — ТОЛЬКО в клетке боя (до 160 px)
 *
 * Четыре ступени говорят «с какой величины видно», и только пятая — «до
 * какой». Она не украшение лестницы, а единственный случай, который иначе
 * невыразим: кружок здоровья заменяет в клетке кружок стоимости, то есть
 * появляется ровно там, где тот исчезает. Сказать это потолком — честно;
 * сказать это одним кружком, меняющим смысл от ширины карты, — значит завести
 * число, которое врёт молча.
 *
 * Ступени, а не тумблер, потому что разница между полкой и клеткой боя — не
 * «бой», а ШИРИНА: 140 пикселей против 400. Карта уже меряет себя
 * контейнерными запросами, и ступень значит ровно то же самое всюду, где карта
 * нарисована, без второго облика, который надо держать в согласии с первым.
 *
 * `cell` дописан снизу, а не переименован: `always` продолжает значить ровно
 * то, что значил, и опись, сохранённая до клетки, не меняет ни одной карты.
 */
export type SheetShow = 'never' | 'large' | 'always' | 'cell' | 'cellOnly';

/** В какой из четырёх полос строка стоит. `over` — «на своём месте поверх
 *  карты»: метка «новая» и подписи под значками не стоят в потоке вовсе. */
export type SheetBand = 'head' | 'props' | 'foot' | 'over';
...
export interface BattleFrame {
    ...
    titleInk: string;
    layout: BattleLayout;
    /** Опись: что печатается, в какой полосе и в каком порядке. Пустая =
     *  домашняя опись, поэтому рамка, сохранённая до описи, ничего не теряет. */
    sheet: SheetRow[];
    /** Кегль всего текста карты, множитель к домашним размерам. 1 — как было. */
    typeScale: number;
    /** Насыщенность второстепенных чернил, множитель. 1 — как было; больше —
     *  темнее, и потолок всё равно непрозрачность. */
    inkFade: number;
    ...
    /** Чем залит кружок, независимо у каждого значка. Пустая строка — как в
     *  доме: стоимость чернилами карты, сила — цветом её каймы. */
    costFill: string;
    powerFill: string;
    /** Чем написана цифра. Пустая строка — «как выйдет»: карта берёт её от
     *  заливки (`badgeInk`). Это УМОЛЧАНИЕ, а не закон: заливки может не быть
     *  вовсе, и тогда цифра лежит на резьбе, про которую ни одна из двух
     *  красок рамы ничего не знает — читаемость там видна только хранителю. */
    costInk: string;
    powerInk: string;
    /** Величина значка — МНОЖИТЕЛЬ к домашним 13cqi, а не размер. Размеры
     *  карта считает из своей ширины; рамка, назначившая пиксели, отняла бы у
     *  неё ровно то, ради чего она их считает. Ноль — «не назначено». */
    costSize: number;
    powerSize: number;
    /** Толщина цифры, 300..800. Ноль — «как у карты». */
    costWeight: number;
    powerWeight: number;
    /** Жетон под цифрой — картинка со склада вместо крашеной подложки. Пустая
     *  строка — жетона нет, значок печатается краской. Жетон СИЛЬНЕЕ формы и
     *  заливки: он и есть нарисованная подложка, и краске под ним делать
     *  нечего. У здоровья своего нет — оно носит жетон стоимости, как носит
     *  её форму, её цвет и её место. */
    costPlate: string;
    powerPlate: string;
    /** Кружок здоровья — тот, что стоит в клетке боя.
     *
     *  Своих полей у него сперва не было: он носил поля стоимости целиком.
     *  Теперь поля свои, но ПУСТОЕ ЗНАЧИТ «как у стоимости», а не «ничего» —
     *  пустая строка, ноль и `null` говорят одно и то же, — поэтому рама,
     *  сохранённая до них, выглядит буква в букву как выглядела, а разводит
     *  их только тот, кто нарочно назначил здоровью своё. */
    healthShape: BattleBadgeShape | '';
    healthFill: string;
    healthInk: string;
    healthPlate: string;
    healthSize: number;
 ...
 *  вернули, и цвет на месте. */
export type BattleBadgeShape = 'circle' | 'square' | 'diamond' | 'hex' | 'shield' | 'none';
