...
pub enum StepId {
    Immunity,
    /// Удары растут к концу партии. Прибавка идёт до всего остального: это не
    /// свойство бьющего и не слабость цели, а то, что время позднее.
    Escalation,
    /// Выстрел дальше своей дальности. У стрелка нет отсечки — есть штраф:
    /// безопасных клеток на поле быть не должно, иначе на них становятся и
    /// стоят.
    LongShot,
    /// Стрелок, к которому подошли вплотную, бьёт вполсилы. Считается до
    /// остального: это не защита цели, а помеха бьющему.
    PointBlank,
    AttackerBless,
    AttackerCurse,
    TargetVulnerable,
